...
export default function Diagnosis({ primaryConsultationId, visitConsultations = [], onReload }) {
  ...
  const openCreate = () => {
    ...
    setForm({ consultationID: primaryConsultationId, isPrimary: false });
  ...
  const records = visitConsultations.flatMap((c) => c.diagnoses || []);
  return (
    <div className="space-y-3">
      {message && <span className="text-sm text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">{message}</span>}
      <div className="flex justify-end">
        <button type="button" disabled={!primaryConsultationId} onClick={openCreate} className="text-sm text-indigo-600 disabled:opacity-40">+ Add diagnosis</button>
      </div>
      {visitConsultations.length === 0 ? (
        ...
        <ClinicalRecordAccordion title="Diagnosis" records={records} idKey="diagnosisID" excludeKeys={["consultationID"]} />
      )}
      {modal && (
        <Modal title="Add diagnosis" onClose={closeModal}>
          <form onSubmit={submit} className="space-y-4">
            <Field label="Code"><input className={inputCls} value={form.code || ""} onChange={(e) => setForm({ ...form, code: e.target.value })} required /></Field>
            <Field label="Description"><textarea className={inputCls} rows={2} value={form.description || ""} onChange={(e) => setForm({ ...form, description: e.target.value })} required /></Field>
            ...
            <Field label="Diagnosis type"><input className={inputCls} value={form.diagnosisType || ""} onChange={(e) => setForm({ ...form, diagnosisType: e.target.value })} /></Field>
            <Field label="Primary">
              <select className={inputCls} value={form.isPrimary ? "true" : "false"} onChange={(e) => setForm({ ...form, isPrimary: e.target.value === "true" })}>

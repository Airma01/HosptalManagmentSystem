...
export default function Immunization() {
  ...
  const load = useCallback(async () => {
    ...
    try { const res = await API.get(`${api}/immunization`); setRows(Array.isArray(res.data) ? res.data : []); }
  ...
  const submit = async (e) => {
    ...
    try {
      await API.post(`${api}/immunization`, { patientVisitID: form.patientVisitID ? Number(form.patientVisitID) : (visitId ? Number(visitId) : null), vaccinationDate: form.vaccinationDate || new Date().toISOString().slice(0,10), vaccineName: form.vaccineName || "", dose: form.dose || null, route: form.route || null, batchNumber: form.batchNumber || null, status: "Given" });
      setShow(false); load();
  ...
  return (
    <div className="space-y-3">
      ...
      {!loading && rows.map((r) => (
        <div key={r.immunizationID} className="bg-white border rounded-xl p-4 text-sm">
          <p className="font-medium">{r.vaccineName} · {r.dose || r.doseNumber || ""}</p>
          <p className="text-xs text-slate-400">{r.vaccinationDate ? new Date(r.vaccinationDate).toLocaleString() : ""} · {r.status}</p>
        </div>
      ))}
      {canWrite && show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <form onSubmit={submit} className="bg-white rounded-xl w-full max-w-md p-5 space-y-3 max-h-[90vh] overflow-y-auto">
            ...
            <input required className={inputCls} placeholder="Vaccine name *" value={form.vaccineName||""} onChange={(e)=>set("vaccineName",e.target.value)} />
            <input type="date" className={inputCls} value={form.vaccinationDate||""} onChange={(e)=>set("vaccinationDate",e.target.value)} />
            <input className={inputCls} placeholder="Dose" value={form.dose||""} onChange={(e)=>set("dose",e.target.value)} />
            <input className={inputCls} placeholder="Route" value={form.route||""} onChange={(e)=>set("route",e.target.value)} />
            <input className={inputCls} placeholder="Batch number" value={form.batchNumber||""} onChange={(e)=>set("batchNumber",e.target.value)} />

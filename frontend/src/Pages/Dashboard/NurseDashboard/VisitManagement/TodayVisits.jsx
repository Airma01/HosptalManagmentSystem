    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <table className="w-full bg-white border rounded-lg">
        ...
        <tbody>
          {visits.map(v => (
            <tr key={v.visitId}>
              <td className="px-4 py-2 border">{v.patientName}</td>
              <td className="px-4 py-2 border">{new Date(v.visitDate).toLocaleString()}</td>
              <td className="px-4 py-2 border">{v.status}</td>
              <td className="px-4 py-2 border">
                <Link to={`/nurse/visits/${v.visitId}`} className="text-blue-500 hover:underline">View</Link>
              </td>
            </tr>
          ))}
        </tbody>

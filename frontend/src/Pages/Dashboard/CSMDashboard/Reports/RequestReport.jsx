...
const RequestReport = () => {
  ...
  const loadData = async (from, to) => {
    try {
      setLoading(true);
      setError(null);
      const data = await getRequestReport(from, to);
      setReport(data);
    } catch (err) {
  ...
  const statusCounts = report?.requests?.reduce((acc, r) => {
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
        ...
        <div className="bg-white shadow rounded-lg p-4 mb-4">
          <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
            ...
            <div className="text-center">
              ...
              <p className="text-xl font-bold text-blue-600">{report.requests.length}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {report?.requests?.length === 0 ? (
        <EmptyState message="No requests found for the selected period." />
      ) : (

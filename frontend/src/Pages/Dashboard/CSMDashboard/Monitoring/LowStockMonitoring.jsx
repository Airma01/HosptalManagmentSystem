...
const LowStockMonitoring = () => {
  ...
  const loadData = async () => {
    try {
      ...
      setError(err.response?.data?.message || "Failed to load low stock medicines.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
      <div className="flex flex-wrap justify-between items-center mb-4">
        <h1 className="text-2xl font-bold text-gray-800">Low Stock Monitoring</h1>
        <span className="text-sm text-red-600">{filtered.length} items low</span>
      </div>

      <div className="mb-4">
        <SearchBar onSearch={handleSearch} placeholder="Search by medicine or generic name..." />
      </div>

      {filtered.length === 0 ? (
        <EmptyState message="No low stock medicines found." />
      ) : (
                ...
                <tr key={item.medicineID}>
                  ...
                  <td className="px-4 py-3 text-sm">
                    {item.isCritical ? (
                      ...
                      <span className="px-2 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800">Low</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-sm">
                    <Link
                      to={`/csm/inventory?medicine=${item.medicineID}`}
